...
import re
import bcrypt
from loginapp1.models import User1, Book
...
def reg_validate(request):
    ...
    if error == True:
        ...
        new_user = User1.objects.create(first_name = request.POST['first_name'], last_name = request.POST['last_name'], email = request.POST['email'], password = pw_hash)
        print(new_user)
...
def login_validate(request):
        ...
        if echeck:
            ...
            if bcrypt.checkpw(request.POST['password'].encode(), echeck[0].password.encode()):
                ...
                print('passwor bad')
                messages.error(request,'Password invalido', extra_tags = 'mal_login_pass_dato')
                return redirect('/')
...
def CreateNewBook(request): # POST /bookss/create
    Book2 = Book.objects.create(title=request.POST['title'], network=request.POST['network'], release_date=request.POST['release_date'], description=request.POST['description'])
...
# *********************************************
# 8 POST /bookss Root Rout redirects to /bookss
